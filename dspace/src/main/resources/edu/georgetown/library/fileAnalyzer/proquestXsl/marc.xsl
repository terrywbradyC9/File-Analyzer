		...
		<marc:record>
			...
			<xsl:for-each select="$pnCatCode">
			    <marc:datafield tag="650" ind1=" " ind2="0">
			       	...
			       	<xsl:variable name="codetext" select="document('lcsh.xml')//lcsh[@code=$code]/val"/>
					<marc:subfield code="a">
			...
			<xsl:for-each select="$pnCatStr">
			  ...
			  <xsl:variable name="item1" select="substring-before($line,';')"/>
			  <xsl:variable name="item23" select="substring-after($line,';')"/>
			  <xsl:variable name="item2" select="substring-before($item23,';')"/>
			  <xsl:variable name="item3" select="substring-after($item23,';')"/>
		      <marc:datafield tag="650" ind1=" " ind2="0">
		        <xsl:choose>
				  <xsl:when test="$item1 = ''">
					<marc:subfield code="a">
